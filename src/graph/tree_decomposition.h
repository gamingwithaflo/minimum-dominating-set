 ...
 struct solution_struct_2 {
     int ref_count;
     std::vector<int> solution;

     solution_struct_2(std::vector<int> sol);
 };
 ...
 public:
     ...
     std::stack<std::unordered_map<std::uint64_t, std::pair<int, solution_struct_2*>>>partial_solution_stack;
     std::unordered_map<std::vector<int>, solution_struct_2, boost::hash<std::vector<int>>>local_solution;
     std::vector<int> global_solution;

 	// Constructor
 	explicit TREE_DECOMPOSITION(std::unique_ptr<NICE_TREE_DECOMPOSITION> nice_tree_decomposition);

     void insert_entry_partial_solution(std::unordered_map<std::uint64_t, std::pair<int, solution_struct_2*>>& partial_solution, std::uint64_t encoding, std::vector<int> key, int size);

     void remove_all_entry_partial_solution(std::unordered_map<std::uint64_t, std::pair<int, solution_struct_2*>>& child_partial_solution_vector);
